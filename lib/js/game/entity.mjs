	...
	.then(async r => await new Promise( (res,rej) => {
		...
		// load the fbs file
			...
			loader.load( fbxURL, function ( object ) {
				...
				// add physics
					let physics = new PHYSICS.Entity(object);
				// get the skeleton for bounding box detection
					let skeleton = new THREE.SkeletonHelper(object);
				// compute the bounding box
					let bbox = _=> new THREE
						.Box3()
						.setFromObject(skeleton)
					let boxhelper = new THREE.BoxHelper(skeleton);
